...
class TestHNSWIndex(unittest.TestCase):
    ...
    def test_neighbor_selection_heuristic(self):
        """Test 14: Ensure the neighbor selection heuristic prefers diverse nodes."""
        # Create a tiny mock graph to bypass search logic and test selection directly
        self.hnsw.vectors = [
            np.array([1.0, 0.0, 0.0]),     # 0: Base node (Query)
            np.array([0.9, 0.435, 0.0]),   # 1: Very close to base
            np.array([0.85, 0.526, 0.0]),  # 2: Redundant (close to base, but closer to node 1)
            np.array([0.85, -0.526, 0.0]), # 3: Diverse (close to base, far from node 1)
        ]
        self.hnsw.ids = ["Q", "C1", "C2", "C3"]
        
        # Candidates list: (similarity_to_base, node_idx)
        candidates = [
            (0.9, 1),
            (0.85, 2),
            (0.85, 3)
        ...
        selected = self.hnsw._select_neighbors(base_node_idx=0, candidates=candidates, M=2)
